...
export class LocationPickerModalComponent {
    ...
    createMap(lat: number, lng: number) {
        ...
        this.map = new gmaps.Map(this.mapContainer.nativeElement, {
            ...
            mapId: environment.googleMapsApiKey, // Using API key as Map ID
    ...
    confirm() {
        if (this.address && this.coords) {
            // Format as address|lat|lng for backend
            const gpsString = `${this.address}|${this.coords.lat}|${this.coords.lng}`;
